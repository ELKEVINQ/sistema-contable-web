...
export class ListaFacturasComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private facturaService: FacturaService, private clienteService: ClienteService, private router: Router) {
  ...
  anularFactura(factura: any) {
    ...
    this.facturaService.anularFactura( {idFactura, estado}).subscribe((response: { success: any; }) => {
      if (response.success){
        alert('Factura anulada correctamente')
      }else{
        alert('Fallo al anular la factura')
      }
    });
    this.reloadPage();
  }
  ...
  imprimirFactura(factura: any) {
    this.clienteService.obtenerCliente(factura.cedula).subscribe((data: any) => {
      // Filtrar el cliente por la cédula
      this.cliente = data[0];
    });

    this.facturaService.obtenerDetalleFactura(factura.idFactura).subscribe((data: any[]) => {
      // Filtrar el cliente por la cédula
    ...
    setTimeout(() => {
      ...
      const autorización = '000000000';
      const ambiente = 'Produccion';
      const emision = 'Normal';//imagen
      ...
      const body = [];
      body.push(['Item', 'Descripcion', 'cantidad', 'P. Unit', 'P. Total'])
      for (const detalle of this.detallesFactura) {
        body.push([
          detalle.numero,
          detalle.descripcion,
          detalle.cantidad,
          (detalle.precioTotal / detalle.cantidad).toFixed(2),
          detalle.precioTotal,
        ]);
      }
      if (factura.estado === 'Valida'){
        ...
        this.estadoTemporal = factura.estado;
      }

      const documentDefinition: any = {
        pageSize: 'A4',
        watermark: { text: this.estadoTemporal, color: 'red', opacity: 0.3, bold: true, italics: false },
        styles: {
          width: 595.28,
          height: '100%',
          header: {
        ...
        content: [
            ...
            columns: [
              ...
              },
              {
                ...
                stack: [
                  'RUC: ' + ruc,
                  {
                    text: 'FACTURA   No. ' + factura.idFactura,
                    fontSize: 15,
                    bold: true
                  },
                  ...
                  autorización,
                  {
                    ...
                    table: {
                      headerRows: 0,
                      body: [
                        [{ text: 'Ambiente: ', bold: true }, { text: ambiente }, { text: 'Emisión: ', bold: true }, { text: emision }],
                      ]
                    },
                  },
                  'Clave de acceso',
          ...
          //Datos del cliente
          {
            ...
            table: {
              ...
              body: [
                [
                  { text: 'Cliente: ' + factura.nombres, alignment: 'left', fontSize: 10 },
                  { text: 'Teléfono: ' + this.cliente.telefono, alignment: 'center', fontSize: 10 },
                  { text: 'Fecha Emision: ' + this.formatearFecha(factura.fecha), alignment: 'right', fontSize: 10 }
                ],
                [
                  { text: 'Cedula/RUC: ' + this.cliente.cedula, alignment: 'left', fontSize: 10 },
                  { text: 'Direccion: ' + this.cliente.direccion, alignment: 'center', fontSize: 10, },
                  { text: '', aligntment: 'right', fontSize: 10, }
                ]
              ]
            }
          },
            ...
            columns: [
              ...
              },
              {
                ...
                table: {
                  ...
                  widths: ['auto', 'auto'],
                  body: [
                    ['SubTotal: ', { text: factura.subtotal, alignment: 'right' }],
                    ['Iva 12%: ', { text: factura.iva, alignment: 'right' }],
                    ['Total: ', { text: factura.total, alignment: 'right' }],
                  ],
                },
              },
            ]
